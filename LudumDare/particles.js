function JSParticle () {
	...
	html5.context.strokeStyle = "rgba("+Math.floor(this.life*this.c[0])+","+Math.floor(this.life*this.c[1])+","+Math.floor(this.life*this.c[2])+","+this.life/this.initlife+")";
	...
	*/
	...
	if (this.life != 0) {
		if (this.initlife/this.life*4.0 > 6)
				if (this.initlife/this.life*4.0 <= 20) {
					html5.context.arc(this.p[0], this.p[1], (this.initlife/this.life)*3.0, 0 , 2 * Math.PI, false);
				} else
					html5.context.arc(this.p[0], this.p[1], 10, 0 , 2 * Math.PI, false);
	}

	html5.context.fillStyle = "rgba("+this.c[1]*255+","+this.c[1]*255+","+this.c[2]*255+","+(this.life/this.initlife)*0.5+")";
	html5.context.fill();
    }


    this.drawLow = function () {
		html5.context.beginPath();
...
function JSParticleEngine () {
	...
	for (i in this.particles) {
	    this.particles[i].drawLow();
	}
	var ctx = html5.context;
    }
}

particles = new JSParticleEngine();
